...
class DatabaseErrorMiddleware(BaseHTTPMiddleware):
    ...
    def handle_postgresql_error(self, error):
        ...
        if sqlstate == "23505":  # UniqueViolation (дубликат)
            return self.response(409, detail_message)

        elif sqlstate == "23502":  # NotNullViolation (обязательное поле)
            return self.response(400, detail_message)

        elif sqlstate == "23503":  # ForeignKeyViolation (неправильный внешний ключ)
            return self.response(400, detail_message)

        elif sqlstate == "23514":  # CheckViolation (проверка CHECK)
            return self.response(400, detail_message)

        elif sqlstate == "23P01":  # ExclusionViolation (исключающее ограничение)
            return self.response(400, detail_message)
